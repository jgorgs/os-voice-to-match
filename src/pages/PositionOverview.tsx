...
import { ThumbsUp, Check, X, Ban, ChevronRight, ChevronDown, Settings, RefreshCw, ArrowLeft } from 'lucide-react';
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h4 className="font-medium text-foreground">Candidate #{candidate.candidate_id}</h4>
            <Badge className={`text-xs ${getScoreColor(candidate.overall_score)}`}>
              {Math.round(candidate.overall_score)}% Match
            </Badge>
          </div>
          
          {/* Mock candidate details - in real app would come from candidates table */}
          <p className="text-sm text-muted-foreground mb-2">Senior Software Engineer at TechCorp</p>
          
          <div className="flex gap-2 mb-3">
            <Badge variant="outline" className="text-xs">
              React {Math.round(candidate.skills_score)}%
            </Badge>
            <Badge variant="outline" className="text-xs">
